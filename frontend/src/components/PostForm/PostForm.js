...
function PostForm({
  ...
  return (
    <div className="form-container">
      ...
      {currentlyPlaying && (
        <div>
          <h2 className="current-song">Currently playing song: {currentlyPlaying.name}</h2>
            ...
            playlists.map((playlist, index) => (
              <optgroup key={index} label={playlist.name}>
                {playlist.tracks.map((song, index) => (
                  <option key={index} value={`${song.title}, ${song.artist}`}>
      ...
      <div className="inputs">
        ...
        <input placeholder="Artist name" value={newArtist} onChange={onChangeArtist} />
      </div>
